...
function SupplierModal({ mode, formData, error, isSaving, onChange, onClose, onSubmit }) {
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="supplier-modal" role="dialog" aria-modal="true" aria-labelledby="supplier-modal-title">
        <div className="modal-header">
          <div>
            <p className="eyebrow">{mode === 'add' ? 'Create' : 'Edit'}</p>
            <h2 id="supplier-modal-title">{mode === 'add' ? 'Add Supplier' : 'Edit Supplier'}</h2>
          </div>
